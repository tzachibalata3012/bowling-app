...
import { GameState, Rolls, ScoreDisplay } from '../Game/Game.type'

import './Scorecard.css'

export default class Scorecard extends Component<Partial<GameState & {totalScore: number}>> {
  renderScores = (frame: number, roll: Rolls) : ScoreDisplay=> {
    const {frames} = this.props
    if(frames)
      return frames[frame] && frames[frame][roll] ? frames[frame][roll].display : ''
    return ''
  }
    ...
    return (
      <div className='Container'>
        <table id='table' className='Scorecard' cellPadding='1' cellSpacing='0'>
          <tbody>
            <tr>
              <th id='c1' colSpan={6}>Round 1</th>
              <th id='c2' colSpan={6}>Round 2</th>
              <th id='c3' colSpan={6}>Round 3</th>
              <th id='c4' colSpan={6}>Round 4</th>
              <th id='c5' colSpan={6}>Round 5</th>
              <th id='c6' colSpan={6}>Round 6</th>
              <th id='c7' colSpan={6}>Round 7</th>
              <th id='c8' colSpan={6}>Round 8</th>
              <th id='c9' colSpan={6}>Round 9</th>
              <th id='c10' colSpan={6}>Round 10</th>
            ...
            <tr>
              ...
              <td id='r19' colSpan={2}>{this.renderScores(9,0)}</td><td id='r20' colSpan={2}>{this.renderScores(9,1)}</td>
              <td id='r21' colSpan={2}>{this.renderScores(9,2)}</td>
